...
From gpfsl.examples.folly Require Import spec_turnSequencer_history code_turnSequencer.
...
Definition TSeqPerm γg l P : vProp :=
  ∃ (γl γs γsl γt : gname),
    meta l nroot (γl,γs,γsl,γt) ∗
    ⎡own γt (set_cf P)⎤.

Definition TSeqComplete γg l (n : nat) : vProp :=
  ∃ (γl γs γsl γt : gname) es el (eVl : omo_event loc_event) V,
    meta l nroot (γl,γs,γsl,γt) ∗
...
Definition TSeqInv γg l E R : vProp :=
  ∃ (γl γs γsl γt : gname) El omo omol stlist mo,
  meta l nroot (γl,γs,γsl,γt) ∗
...
Definition TSeqLocal (_ : namespace) γg l (E : history tseq_event) M : vProp :=
  ∃ (γl γs γsl γt : gname) Ml,
  meta l nroot (γl,γs,γsl,γt) ∗
...
Global Instance TSeqInv_objective γg l E R : Objective (TSeqInv γg l E R) := _.
Global Instance TSeqPerm_objective γg l P : Objective (TSeqPerm γg l P) := _.
Global Instance TSeqLocal_persistent N γg l E M : Persistent (TSeqLocal N γg l E M) := _.
...
Lemma TSeqInv_TSeqLocal_instance :
  ∀ N γg l E E' M' R,
    TSeqInv γg l E R -∗ TSeqLocal N γg l E' M' -∗ ⌜ E' ⊑ E ⌝.
Proof.
  intros. iDestruct 1 as (?????????) "(HM & M● & _)".
  iDestruct 1 as (?????) "(HM' & E◯ & _)". simplify_meta with "HM' HM".
  by iDestruct (OmoAuth_OmoSnapHistory with "M● E◯") as %?.
Qed.

Lemma TSeqLocal_lookup_instance :
  ∀ N γg l E M e V,
    sync <$> E !! e = Some V → e ∈ M →
    TSeqLocal N γg l E M -∗ ⊒V.
Proof.
...
Proof.
  ...
  iMod (append_only_loc_swriter_from_na_rel with "RV l↦") as (γl γsl V1 eVl0)
    "(#⊒V1 & Ml● & (#Ml◯ & [R@V1 #⊒V0@V1] & SW) & omol↦ & WCOMMIT & #el0✓eVl0 & Pures)"; [done|].
  ...
  iMod (@OmoAuth_alloc _ _ _ _ _ eVinit initst _ _ tseq_interpretable with "WCOMMIT") as (γg γs) "(M● & #M◯ & #e0↦el0 & #e0✓eVinit & #e0↪initst & _)".
  ...
  iMod (meta_set _ _ (γl,γs,γsl,γt) nroot with "Hml") as "#HM"; [done|].
  rewrite shift_0.

  iDestruct ("Post" $! γg l [eVinit] {[0%nat]} V1 with "[-]") as "HΦ"; last first.
  { iModIntro. by iApply "HΦ". }

  iFrame "⊒V1". iSplitR; last iSplitR "Hγt2"; last iSplitL.
  - repeat iExists _. iFrame "HM M◯ E◯ Ml◯".
...
Proof.
  intros N DISJ l tid γg E0 M V0 n R. iIntros "#⊒V0 #TSeqLocal Perm" (Φ) "AU".
  iDestruct "TSeqLocal" as (???? Ml0) "(HM & E◯0 & M◯0 & Ml◯0)".
  iLöb as "IH". wp_lam. wp_bind (!ᵃᶜ _)%E.

  (* Inv access *)
  iMod "AU" as (E1) "[TSeqInv Close]".
  iDestruct "TSeqInv" as (???? El1 omo1 omol1 stlist1 mo1)
  ...
  iIntros (el1 genl1 vl1 Vl1 V1 eVl1 eVln1) "(Pures & Ml●1 & RCOMMIT & #MAXgen_el1 & #el1✓eVl1 & #eln1✓eVln1 & #el1=eln1 & #⊒V1 & #Ml◯1' & omol↦1)".
  ...
  { (* Wait failed, retry *)
    ...
    { iDestruct "LAST1" as (elf1 eVlf1 Vf1 eVf1 stf1) "(#elf1✓eVlf1 & #ef1✓eVf1 & #ef1↪stf1 &
    ...
    iModIntro. wp_pures. rewrite bool_decide_false; [|lia]. wp_op. rewrite bool_decide_false; [|lia]. wp_if. iApply ("IH" with "Perm AU"). }
  ...
  iAssert (OmoUB γg M ef1)%I with "[-]" as "#MAXgen_ef1_p1".
  { iAssert (⌜ OmoUBgen omo1 M (length omo1 - 1)%nat ⌝)%I with "[-]" as %MAXgen.
    { iIntros "%e %eM". iDestruct (OmoAuth_OmoEview with "M●1 M◯0") as %Incl.
      apply Incl in eM. iDestruct (OmoAuth_wf with "M●1") as %[OMO_GOOD _]. iDestruct (OmoAuth_omo_nonempty with "M●1") as %Nomo.
      eapply lookup_omo_surjective in eM; [|done]. destruct eM as [eidx Heidx]. iPureIntro. exists eidx. split; [done|].
      apply lookup_omo_lt_Some in Heidx. destruct omo1; try done. lia. }
    iApply (OmoUB_from_gen _ _ _ _ _ _ _ _ _ (wr_event (length omo1 - 1)%nat) with "M●1"); [|done|done].
    rewrite -LASTef1 last_lookup lookup_omo_wr_event -omo_write_op_length.
    replace (Init.Nat.pred (length omo1)) with (length omo1 - 1)%nat by lia. done. }
  iDestruct (OmoUB_singleton with "ef1✓eVf1") as "#MAXgen_ef1_p2".
  iDestruct (OmoUB_union with "MAXgen_ef1_p2 MAXgen_ef1_p1") as "MAXgen_ef1".
  ...
  iMod (OmoAuth_insert_ro _ _ _ _ _ _ _ _ _ _ egV' with "M●1 M◯1@V1 RCOMMIT ef1↪stf1 MAXgen_ef1 []")
    as (gen1') "(M●1 & #M◯0'@V1 & #waitId↦eln1 & #ef1=waitId & #waitId✓egV' & _ & %HwaitId)".
  { have ? : step waitId egV' stf1 stf1.
    { destruct stf1 as [[[a b] c] d]. simpl in *. subst b c d.
      apply tseq_step_Take.
      - by subst egV'.
    ...
    subst egV' M' waitId ef1. rewrite stf1EVIEW /=. set_solver-. }
  ...
  iMod ("Commit" $! V1 E1' M' with "[-RES PermReturn]") as "HΦ".
  { iFrame "⊒V1". iSplitL; last iSplit.
    - repeat iExists _. iFrame "HM M●1 Ml●1". iSplitL "omol↦1"; [eauto with iFrame|].
      rewrite omo_insert_r_write_op. iFrame "AllWrEvents1". iExists el1,eVl1,_,_,stf1. iFrame "el1✓eVl1 ef1✓eVf1 ef1↪stf1 LTPerm".
      iSplitR; [|by iRight]. iPureIntro. split_and!; try done.
    ...
    - repeat iExists _. iFrame "HM E◯1 Ml◯1'". subst M'.
...
Proof.
  intros N DISJ l tid γg E0 M V0 n R. iIntros "#⊒V0 #TSeqLocal RSN CompleteN" (Φ) "AU".
  iDestruct "TSeqLocal" as (???? Ml0) "(HM & E◯0 & M◯0 & Ml◯0)".
  wp_lam. wp_bind (!ʳˡˣ _)%E.

  (* Inv access 1 *)
  iMod "AU" as (E1) "[TSeqInv [Abort _]]".
  iDestruct "TSeqInv" as (???? El1 omo1 omol1 stlist1 mo1)
    "(>HM' & >M●1 & >Ml●1 & (%Vb1 & %uf1 & >omol↦1) & >#AllWrEvents1 & LAST1)".
  simplify_meta with "HM' HM". iClear "HM'".
  ...
  (* Inv access 2 *)
  iMod "AU" as (E2) "[TSeqInv [_ Commit]]".
  iDestruct "TSeqInv" as (???? El2 omo2 omol2 stlist2 mo2)
  ...
  wp_apply (append_only_loc_release_write _ _ _ _ _ _ _ _ _ _ _ _ (n + 1)%Z (R (n + 1)%nat) with "[$Ml●2 $Ml◯0 $SW $omol↦2 $RSN $⊒V1]");
    [solve_lat|done|].
  ...
  iDestruct "Pures" as %(LASTelf1' & LASTef1 & eVlf1'EV & eVlf1'SYNC & LASTstf1 & stf1VAL & stf1VIEW & eVf1COMM & stf1EVIEW).
  ...
  iDestruct (OmoEview_insert_new_obj with "M◯0@V2 ef1✓eVf1") as "#M◯2@V2"; [by rewrite -eVlf1'SYNC; solve_lat|].
  ...
  iMod (OmoAuth_insert_last with "M●2 M◯2@V2 WCOMMIT []")
    as "(M●2 & #M◯2'@V2 & #compId↦eln2 & #compId✓egV' & #compId↪stf2 & _)".
  { have ? : step compId egV' stf1 stf2.
    { destruct stf1 as [[[a b] c] d]. simpl in *. subst stf2 egV' M' c.
      ...
      rewrite -eVlf1'SYNC. clear -LeeVlf1Vlf LeVlf1V0' LeV0'Vl1V1 LeV1V2. solve_lat. }
  ...
  iMod ("Commit" $! V2 E2' M' with "[-]") as "HΦ"; last first. { iModIntro. by iApply "HΦ". }
  ...
  - repeat iExists _. iFrame "HM M●2 Ml●2". rewrite omo_append_w_write_op. subst E2'.
    ...
    + iExists (length El2),eVln2,V2,egV',stf2. subst stf2. simpl. iFrame "eln2✓eVln2 compId✓egV' compId↪stf2 LTPerm".
      ...
      * rewrite omo_append_w_write_op last_app. done.
      * rewrite eVln2EV. have EQ : (Z.of_nat stf1.1.1.2 + 1)%Z = (Z.of_nat (stf1.1.1.2 + 1)) by lia. by rewrite EQ.
      * rewrite app_length stf1VAL /=. lia.
  - repeat iExists _. subst E2' M'. iFrame "HM E◯2 Ml◯2@V2". rewrite stf1EVIEW.
...
Definition tseq_impl `{!noprolG Σ, !atomicG Σ, !tseqG Σ}
  : tseq_spec Σ := {|
    spec_turnSequencer_history.TSeqInv_Linearizable := TSeqInv_Linearizable_instance ;
    spec_turnSequencer_history.TSeqInv_history_wf := TSeqInv_history_wf_instance;
    spec_turnSequencer_history.TSeqInv_TSeqLocal := TSeqInv_TSeqLocal_instance;
    spec_turnSequencer_history.TSeqLocal_lookup := TSeqLocal_lookup_instance;
    spec_turnSequencer_history.TSeqPerm_Equiv := TSeqPerm_equiv;
    spec_turnSequencer_history.TSeqPerm_Split := TSeqPerm_split;
    spec_turnSequencer_history.TSeqPerm_Combine := TSeqPerm_combine;
    spec_turnSequencer_history.TSeqPerm_Excl := TSeqPerm_excl;

    spec_turnSequencer_history.new_tseq_spec := new_tseq_spec;
    spec_turnSequencer_history.wait_spec := wait_spec;
    spec_turnSequencer_history.complete_spec := complete_spec;
  |}.
