...
Definition QueueInv γg γs q E omo stlist : vProp := OmoAuth γg γs (1/2)%Qp E omo stlist _.
...
Definition atomic_update_deq (N : namespace) γg q (V : view) (M : eView) (Q : Z → vProp) : vProp :=
  atomic_update (⊤ ∖ ↑N) (↑histN)
    ...
    (tele_app (TT:= [tele _ _ _ _])
      (λ γs E omo stlist, tele_app (TT:= [tele _ _ _ _])
        (λ (v: Z) V' st M',
          let eVdeq := mkOmoEvent (Deq v) V' M' in
          let E' := E ++ [eVdeq] in
          let omo' := omo_append_w omo (length E) [] in
          (* PUBLIC POST *)
          ⊒V' ∗ ▷ QueueInv γg γs q E' omo' (stlist ++ [st]) ∗ @{V'} QueueLocal_inner N γg q M' ∗
...
Proof.
  iIntros (N tid sz Vi NZSZ Φ) "#⊒Vi Post".
  ...
  iMod (append_only_loc_cas_only_from_na_rel with "⊒Vi q0↦") as (γl0 γsl0 V0 eVl0)
    "(#⊒V0 & Ml0● & [#⊒Ml0 #⊒V0@Vi] & omol0↦ & WCOMMIT & #el0✓eVl0 & [%eVl0EV _])"; [done|].
  iMod (append_only_loc_cas_only_from_na_rel with "⊒V0 q1↦") as (γl1 γsl1 V1 eVl1)
    "(#⊒V1 & Ml1● & [#⊒Ml1 #⊒V1@V0] & omol1↦ & _ & #el1✓eVl1 & [%eVl1EV _])"; [done|].
  iAssert (⌜ Vi ⊑ V0 ∧ V0 ⊑ V1 ⌝)%I with "[]" as %[LeViV0 LeV0V1].
  { rewrite !view_at_unfold !monPred_at_in. iDestruct "⊒V0@Vi" as %?. by iDestruct "⊒V1@V0" as %?. }
  ...
  { rewrite bool_decide_false; [|lia]. wp_if.
    ...
    - rewrite /slot_field big_sepL_snoc. iFrame "SF".
      iExists γseq,γseqs,sloc. iSplit; [done|]. repeat iExists _. iFrame "Mseq●". iSplit; [done|]. done.
  ...
  have LeViV' : Vi ⊑ V' by solve_lat.
  set M := {[0%nat]}.
  set eVinit := mkOmoEvent Init V' M.

  iMod (@OmoAuth_alloc _ _ _ _ _ eVinit [] _ _ queue_interpretable with "WCOMMIT")
    as (γg γs) "(M● & #⊒M & _ & #e0✓eVinit & _ & WCOMMIT)".
  ...
  iMod (inv_alloc (mpmcqN N) _ (InternalInv_inner N γg q)  with "[-M●' Post WCOMMIT]") as "#I".
  ...
  wp_pures. iApply ("Post" $! γg γs q V' {[0]}). iFrame "⊒V' M●' WCOMMIT". iSplit; [|done].
  iFrame "I". repeat iExists _. iFrame "HM ⊒AOLs@V ⊒SEQs@V ⊒M". iExists eVinit. iFrame "e0✓eVinit". done.
Qed.
...
Proof.
  intros N DISJ q tid γg M V0 v NZ. iIntros "#⊒V0 #QueueLocal" (Φ) "AU".
  iDestruct "QueueLocal" as "[Inner QInv]".
  iDestruct "Inner" as (??????) "(HM & ⊒M & ⊒AOLs & ⊒SEQs & ⊒eV0)".
  iCombine "⊒M ⊒AOLs ⊒SEQs ⊒eV0" as "⊒MAOLSEQ".
  iDestruct (view_at_intro_incl with "⊒MAOLSEQ ⊒V0")
    as (V0') "(⊒V0' & %LeV0V0' & (⊒M@V0' & ⊒AOLs@V0' & ⊒SEQs@V0' & ⊒eV0@V0'))".
  ...
  { iExists E1. do 6 (iExists _). iExists enqcnt1, deqcnt1, omo1, stlist1, nodes1, hlist1.
    iFrame "M●1 NL●1 HL●1 HM H". repeat iExists _.
    iFrame "Ml1●". rewrite omo_insert_r_write_op. iFrame "MSG1".
  ...
  { (* CAS in FAA failed, retry FAA *)
    ...
    { iExists E2. do 6 (iExists _). iExists enqcnt2, deqcnt2, omo2, stlist2, nodes2, hlist2.
      iFrame "M●2 NL●2 HL●2 HM H". repeat iExists _. iFrame "Ml2●".
      subst omol2'. rewrite omo_insert_r_write_op. iFrame "MSG2".
      iSplitR; [|eauto with iFrame]. rewrite omo_insert_r_length. done. }
  ...
  iMod (OmoAuth_insert_last with "M●2 ⊒M@V2 WCOMMIT []")
    as "(M●2 & #⊒M'@V2 & #enqId↦eln2 & #enqId✓eVenq & #enqId↪enqst & WCOMMIT)".
  { have ? : step enqId eVenq (drop deqcnt2 nodes2) enqst.
    { by apply queue_step_Enq; try done; subst eVenq M'; set_solver-. }
    iPureIntro. by split_and!. }
  iDestruct (OmoHbToken_finish with "M●2") as "[M●2 M●2']".
  iDestruct (OmoUB_singleton with "enqId✓eVenq") as "MAXenqId".
  iDestruct (big_sepS_subseteq _ _ M with "MAXenqId") as "MAXenqId'"; [set_solver-|].

  iMod ("Commit" $! V2 enqst M' with "[M●2' WCOMMIT]") as "HΦ".
  { iFrame "⊒V2 M●2' WCOMMIT MAXenqId'". iSplit.
    - iFrame "QInv". repeat iExists _. iFrame "HM ⊒M'@V2 ⊒AOLs@V0' ⊒SEQs@V0' ⊒eV0@V0'".
  ...
  { destruct (le_lt_dec deqcnt2 enqcnt2) as [LE|LT].
    ...
    iAssert (|={_}=> @{Vb ⊔ V2} (▷ Q v ∗ ⎡ghost_var γi 1 Accepted⎤ ∗
      (∃ E2'' omo2'' stlist2'', OmoAuth γg γs (1/2) E2'' omo2'' stlist2'' _ ∗
        ⌜ last stlist2'' = Some (drop deqcnt2 nodes2') ⌝)))%I with "[AU Hγi WCOMMIT M●2]"
          as ">(HQ & [Hγi Hγi'] & (%&%&%& M● & %LAST))".
    { rewrite -view_at_fupd.
      ...
      iMod (OmoAuth_insert_last with "M● ⊒M WCOMMIT []") as "(M● & #⊒M' & _ & #deqId✓eVdeq & _ & WCOMMIT)".
      { have ? : step deqId eVdeq enqst (drop deqcnt2 nodes2).
        { subst enqst. rewrite drop_ge; [|lia]. simpl.
      ...
      iDestruct (OmoUB_singleton with "deqId✓eVdeq") as "MAXdeqId".
      iDestruct (big_sepS_subseteq _ _ M0 with "MAXdeqId") as "MAXdeqId'"; [set_solver-|].

      iMod ("Commit" $! v (Vb ⊔ V2) _ M0'' with "[M●' WCOMMIT]") as "HΦ".
      { iFrame "⊒V M●' WCOMMIT MAXdeqId'". iSplit.
  ...
  { iDestruct "Helping2" as (???) "(Helping2 & M● & %LAST)".
    iExists E2''. repeat iExists _.
    iFrame "HM M● NL●2' HL●2 Helping2 Field2' Slot2 Pdeq2 Tdeq2 Penq2 Hnodes2".
    iSplitL; last iSplit.
  ...
  iDestruct "Field2" as (??) "[#e0✓eV0 [q↦sz BIGq]]".
  iDestruct "⊒eV0" as (?) "[e0✓eV0' ⊒eV0OUT]".
  iDestruct (OmoEinfo_agree with "e0✓eV0 e0✓eV0'") as %<-.
  iDestruct (view_at_elim with "⊒eV0OUT q↦sz") as "q↦sz".
  iAssert (@{eV0.(sync)} (q >> (Z.to_nat idx + 3)) ↦{qp} #(LitLoc sloc))%I with "[BIGq]" as "q↦sloc".
  { iApply (view_at_mono with "BIGq"); [done|]. iIntros "BIG".
    iDestruct (big_sepL_lookup with "BIG") as "H"; [done|]. done. }
  ...
  { (* Peeking case *)
    ...
    iApply "Slot3Close". repeat iExists _. iSplit; [done|].
    repeat iExists _. iFrame "Ms3● Hstfs3". done. }
  ...
  iExists E3. repeat iExists _.
  iFrame "HM M●3 NL●3 HL●3 Tenq3 Tdeq3 Field3 Penq3 Pdeq3 Helping3 Hnodes3".
...
Proof.
  ...
  { iExists E1. do 6 (iExists _). iExists enqcnt1, deqcnt1, omo1, stlist1, nodes1, hlist1.
    iFrame "M●1 NL●1 HL●1 HM Tenq1 H". repeat iExists _.
    iFrame "Ml1●". rewrite omo_insert_r_write_op. iFrame "MSG1".
    iSplitR; [|eauto with iFrame]. rewrite omo_insert_r_length. done. }
  ...
  { (* CAS in FAA failed, retry FAA *)
    ...
    iMod ("Hclose" with "[-AU]") as "_".
    { iExists E2. do 6 (iExists _). iExists enqcnt2, deqcnt2, omo2, stlist2, nodes2, hlist2.
      iFrame "M●2 NL●2 HL●2 HM H Tenq2". repeat iExists _.
      iFrame "Ml2●". subst omol2'. rewrite omo_insert_r_write_op. iFrame "MSG2".
      iSplitR; [|eauto with iFrame]. rewrite omo_insert_r_length. done. }
    iModIntro. wp_if. iApply ("IH" with "AU"). }
  ...
  iDestruct "sVw2" as (Vw2 (Eqmol2 & Homol2 & eVln2EV & eVln2SYNC & LeVl2Vw3 & NEqVl2Vw2 & NLeV2Vl2 & NEqV1V2 & LeEmpVw2))
    "(_ & _ & WCOMMIT)".
  iClear "IH". iDestruct "H"
    as "(Field2 & Slot2 & Penq2 & Pdeq2 & Helping2 & #Hnodes2 & (%LASTst2 & %LENnodes2 & %LENhlist2 & %LENseqs & %LENaols & %NZsz))".
  iDestruct (field_info_dup with "Field2") as "[Field2 Field2']".
  ...
  { destruct (le_lt_dec enqcnt2 deqcnt2) as [LE|LT].
    - (* Matching enq has not been committed → need helping *)
      ...
      { rewrite /atomic_update_deq. iAuIntro. iApply (aacc_aupd with "AU"); [done|].
        iIntros (γs' E omo stlist) "H". iAaccIntro with "H"; [eauto with iFrame|].
        iIntros (v V' st M') "(⊒V' & M● & ⊒M'@V' & WCOMMIT & MAX & Pures) !>". iRight.
        iExists v,V',st,M'. iFrame. iFrame "QInv". iSplit; [done|]. iIntros "HΦ". iModIntro. wp_pures. by iApply "HΦ". }
    ...
    - (* Matching enq has already been committed → commit immediately *)
      ...
      { rewrite -view_at_fupd.
        ...
        iMod "AU" as (????) "[>M●' [_ Commit]]".
        iDestruct (OmoAuth_agree with "M● M●'") as %(<- & <- & <- & <-). iCombine "M● M●'" as "M●".
        iMod (OmoAuth_insert_last with "M● ⊒M'@V2'' WCOMMIT []")
          as "(M● & #⊒M''@V2'' & _ & #deqId✓eVdeq & _ & WCOMMIT)".
        { have ? : step deqId eVdeq ((enqId, v, eVenq.(sync), eVenq.(eview)) :: deqst) deqst.
          - apply queue_step_Deq; try done; [solve_lat|set_solver-].
          - rewrite Hdeqst in LASTst2. iPureIntro. split_and!; try done. }
        iDestruct (OmoHbToken_finish with "M●") as "[M● M●']".
        iDestruct (OmoUB_singleton with "deqId✓eVdeq") as "MAXdeqId".
        iDestruct (big_sepS_subseteq _ _ M with "MAXdeqId") as "MAXdeqId'"; [set_solver-|].

        iMod ("Commit" $! v V2'' _ M'' with "[M●' WCOMMIT]") as "HΦ".
        { iFrame "⊒V2'' M●' WCOMMIT MAXdeqId'". iSplit.
          - iFrame "QInv". repeat iExists _. iFrame "HM ⊒M''@V2'' ⊒AOLs@V2'' ⊒SEQs@V2'' ⊒eV0@V2''".
          - iPureIntro. split_and!; try done. set_solver-. }
      ...
      iModIntro. iExists γi,γu,V2',E2',omo2',stlist2'.
      iFrame "M● HL●2 HInv ⊒V2' Hγu". iSplitR; [done|]. iSplitL; [|done].
  ...
  { eapply (omo_write_op_step _ _ _ (length omos3 - 1)); [apply OMO_GOOD'|..].
    ...
    - rewrite (omo_stlist_length _ _ _ OMO_GOOD).
      rewrite lookup_app_l; [|destruct sts3; [done|simpl;lia]].
