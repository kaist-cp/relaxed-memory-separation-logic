...
From gpfsl.examples.folly Require Import spec_turnSequencer_composition spec_singleElementQueue_history code_turnSequencer code_singleElementQueue.
...
Definition SeqPerm γg q ty P : vProp :=
  ∃ γts γs γv γsts γti tsloc,
    meta q nroot (γts,γs,γsts,γv,γti,tsloc) ∗
    match ty with
...
Definition SeqInv γg q E : vProp :=
  ∃ γts γs γsts γv γti tsloc (qg : Qp) Et omo omot stlist tstlist,
    meta q nroot (γts,γs,γsts,γv,γti,tsloc) ∗
...
Definition SeqLocal (N : namespace) γg q (E : history seq_event) M : vProp :=
  ∃ γts γs γsts γv γti tsloc Mt (eV0 : omo_event seq_event),
    meta q nroot (γts,γs,γsts,γv,γti,tsloc) ∗
...
Proof.
  ...
  destruct x; apply _.
Qed.
Global Instance SeqInv_objective γg q E : Objective (SeqInv γg q E) := _.
...
Global Instance SeqLocal_persistent N γg q E M : Persistent (SeqLocal N γg q E M) := _.
...
Lemma SeqInv_SeqLocal_instance :
  ∀ N γg q E E' M',
...
Lemma SeqLocal_lookup_instance :
  ∀ N γg q E M e V,
    sync <$> E !! e = Some V → e ∈ M →
    SeqLocal N γg q E M -∗ ⊒V.
Proof.
...
Proof.
  ...
  iMod (@OmoAuth_alloc _ _ _ _ _ eVinit initst _ _ seq_interpretable with "WCOMMIT") as (γg γs) "(M● & #M◯ & _ & #e0✓eVinit & #e0↪initst & _)".
  ...
  iMod (meta_set _ _ (γts,γs,γsts,γv,γti,tsloc) nroot with "Hmq") as "#HM"; [done|].
  ...
  iDestruct ("Post" $! γg q [eVinit] {[0]} V2 with "[-]") as "HΦ"; [|iModIntro; by iApply "HΦ"].
  iFrame "⊒V2 EnqPerm DeqPerm". iSplitR; last iSplitL; [..|done].
  - repeat iExists _. iFrame "HM E◯ TSeq◯ M◯ e0✓eVinit". subst eVinit. simpl. done.
...
Proof.
  intros N DISJ q tid γg E0 M V0 v n vPos. iIntros "#⊒V0 #SeqLocal Perm" (Φ) "AU".
  iDestruct "SeqLocal" as (?????? Mt0 eV0) "(HM & E◯0 & M◯0 & TSeq◯0 & e0✓eV0 & #⊒eV0SYNC)".
  ...
  (* Inv access 1 *)
  iMod "AU" as (E1) "[SeqInv [Abort1 _]]".
  iDestruct "SeqInv" as (?????? qg1 Et1 omo1 omot1) "(%stlist1 & %tstlist1 & >HM' & >M●1 & TSeq●1 & >Field1 & >LAST1)".
  ...
  iAssert (TSeqPerm ts γts tsloc (λ m, m =? n * 2) ∗ ⎡own γti (set_cf (λ m, m =? n * 2))⎤)%I with "[Perm]" as "[TSeqPerm PermInternal]".
  ...
  (* Inv access 2 *)
  iApply (aacc_aupd with "AU"); [solve_ndisj|]. iIntros (E2) "SeqInv".
  iDestruct "SeqInv" as (?????? qg2 Et2 omo2 omot2) "(%stlist2 & %tstlist2 & >HM' & >M●2 & TSeq●2 & Field2 & >LAST2)".
  ...
  { rewrite !last_lookup in LASTtstf2.
    ...
    { rewrite lookup_lt_is_Some -omo_read_op_length.
      apply lookup_lt_Some in LASTtstf2. rewrite -(omo_stlist_length _ _ _ OMO_GOODts2) in LASTtstf2. done. }
    have LOOKUP2 : lookup_omo omot2' (ro_event (length omot2 - 1) (length es)) = Some (length Et2).
    { subst omot2'. rewrite lookup_omo_ro_event omo_insert_r_read_op list_lookup_alter Hes /= lookup_app_1_eq. done. }
  ...
  awp_apply (complete_spec ts _ DISJ tsloc _ _ γsts Mt0 V2 (n * 2) (TSeqRes γv q) with "⊒V2 TSeq◯0 [q↦1 Hγv'] TSeqComplete").
  { iExists v. iFrame. }

  (* Inv access 3 *)
  iApply (aacc_aupd with "AU"); [solve_ndisj|]. iIntros (E3) "SeqInv".
  iDestruct "SeqInv" as (?????? qg3 Et3 omo3 omot3) "(%stlist3 & %tstlist3 & >HM' & >M●3 & TSeq●3 & >Field3 & >LAST3)".
  simplify_meta with "HM' HM". iClear "HM'".
  ...
  iMod (OmoAuth_insert_last with "M●3 M◯0@V3 WCOMMIT []") as "(M●3 & #M◯3@V3 & #enqId↦etn3 & #enqId✓eVenq & #enqId↪enqst & _)".
  ...
  iModIntro. iRight. iExists V3,E2',M'. iFrame "⊒V3". iSplitR "PermReturn"; last first.
  ...
  - repeat iExists _. iFrame "HM M●3 TSeq●3 Field3".
    ...
    + iRight. iSplitR; [done|]. subst enqst. simpl. iExists eVenq. iFrame "enqId✓eVenq Hγv". iPureIntro. split_and!; try done.
      ...
      * lia.
  - repeat iExists _. iFrame "HM E◯3 TSeq◯3@V3 M◯3@V3 e0✓eV0 ⊒eV0SYNC@V0'".
...
Proof.
  intros N DISJ q tid γg E0 M V0 n. iIntros "#⊒V0 #SeqLocal Perm" (Φ) "AU".
  iDestruct "SeqLocal" as (?????? Mt0 eV0) "(HM & E◯0 & M◯0 & TSeq◯0 & e0✓eV0 & ⊒eV0SYNC)".
  iCombine "M◯0 TSeq◯0 ⊒eV0SYNC" as "Frags".
  iDestruct (view_at_intro_incl with "Frags ⊒V0") as (V0') "(⊒V0' & %LeV0V0' & (M◯0@V0' & TSeq◯0@V0' & ⊒eV0SYNC@V0'))".
  wp_lam. wp_op. rewrite -[Z.to_nat 0]/(0%nat) shift_0. wp_bind (! _)%E.
  ...
  wp_read. iApply fupd_mask_intro_subseteq; [done|]. wp_pures.
  iDestruct (SeqPerm_split _ _ _ _ (λ m, m =? n) with "Perm") as "[Perm PermReturn]".
  iAssert (TSeqPerm ts γts tsloc (λ m, m =? n * 2 + 1) ∗ ⎡own γti (set_cf (λ m, m =? n * 2 + 1))⎤)%I with "[Perm]" as "[TSeqPerm PermInternal]".
  { iDestruct "Perm" as (??????) "[HM' [Perm PermInternal]]". simplify_meta with "HM' HM". iClear "HM'".
    ...
    { intros. destruct (decide (m = n * 2 + 1)) as [->|NEQ].
      - have EQ1 : Nat.odd (n * 2 + 1) = true.
        { rewrite Nat.odd_spec. apply Nat.Odd_add_r; [|by rewrite -Nat.odd_spec]. apply Nat.Even_mul_r. by rewrite -Nat.even_spec. }
  ...
  awp_apply (wait_spec ts _ DISJ tsloc _ _ γsts Mt0 V0' (n * 2 + 1) (TSeqRes γv q) with "⊒V0' TSeq◯0 TSeqPerm").
  ...
  iDestruct "LAST2" as (stf2 tstf2) "[(%LENeq2 & %LASTstf2 & %LASTtstf2) CASE]".
  iAssert (⌜ tstf2.1.1.2 = (n * 2 + 1)%nat ∧ tstf2.1.2 ⊑ V2 ⌝)%I with "[-]" as %[Eqtstf2_1_1_2 Letstf2_1_2V2].
  { rewrite !last_lookup in LASTtstf2.
    replace (Init.Nat.pred (length tstlist2)) with (length tstlist2 - 1) in LASTtstf2 by lia.
    rewrite -(omo_stlist_length _ _ _ OMO_GOODts2) in LASTtstf2.
    have LOOKUP1 : (Et2 ++ [eVwait]) !! (length Et2) = Some eVwait by apply lookup_app_1_eq.
  ...
  iDestruct "Hstf2" as (eVf2) "(#ef2✓eVf2 & Hγv' & (%LASTef2 & % & %Oddtstf2 & %EqTurn & %GT & %Eqstfh2tstf2 & %EqViewstfh2 & %EqLviewstfh2))".
  ...
  iMod (OmoAuth_insert_last with "M●3 M◯0'@V3 WCOMMIT []")
    as "(M●3 & #M◯3@V3 & #enqId↦etn3 & #enqId✓eVenq & #enqId↪enqst & _)".
  ...
  iModIntro. iRight. iExists V3,E2',M',stfh2.1.1.1.2. iFrame "⊒V3". iSplitL; last first.
  ...
  - repeat iExists _. iFrame "HM E◯3 TSeq◯3@V3 e0✓eV0 ⊒eV0SYNC@V0'".
    replace ({[length E2]} ∪ ({[stfh2.1.1.1.1]} ∪ eVf2.(eview) ∪ M)) with M' by set_solver. iFrame "M◯3@V3".
  - iPureIntro. split_and!; try done. subst M'. set_solver-.
Qed.
End Seq.

Definition seq_impl `{!noprolG Σ, !atomicG Σ, !seqG Σ} (ts : tseq_composition_spec Σ)
  : seq_spec Σ := {|
    spec_singleElementQueue_history.SeqInv_Linearizable := SeqInv_Linearizable_instance ts;
    spec_singleElementQueue_history.SeqInv_history_wf := SeqInv_history_wf_instance ts;
    spec_singleElementQueue_history.SeqInv_SeqLocal := SeqInv_SeqLocal_instance ts;
    spec_singleElementQueue_history.SeqLocal_lookup := SeqLocal_lookup_instance ts;
    spec_singleElementQueue_history.SeqPerm_Equiv := SeqPerm_equiv ts;
    spec_singleElementQueue_history.SeqPerm_Split := SeqPerm_split ts;
    spec_singleElementQueue_history.SeqPerm_Combine := SeqPerm_combine ts;
    spec_singleElementQueue_history.SeqPerm_Excl := SeqPerm_excl ts;

    spec_singleElementQueue_history.new_seq_spec := new_seq_spec ts;
    spec_singleElementQueue_history.enqueueWithTicket_spec := enqueueWithTicket_spec ts;
    spec_singleElementQueue_history.dequeueWithTicket_spec := dequeueWithTicket_spec ts;
  |}.
