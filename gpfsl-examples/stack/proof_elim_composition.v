...
From gpfsl.examples.stack Require Import spec_elim_composition spec_treiber_composition code_elim.
...
Definition EStackInv γg γs (s : loc) E omo stlist : vProp := OmoAuth γg γs (1/2)%Qp E omo stlist _.
...
Definition atomic_update_push (N : namespace) γg s (v : Z) (V : view) (M : eView) (Q : bool → vProp) : vProp :=
  atomic_update (⊤ ∖ ↑N) (↑histN)
    ...
    (tele_app (TT:= [tele _ _ _ _])
      (λ γs E omo stlist, tele_app (TT:= [tele _ _ _ _ _ _ _])
        (λ (b: bool) V' γs' E' omo' stlist' M',
          (* PUBLIC POST *)
          ⊒V' ∗ ▷ EStackInv γg γs' s E' omo' stlist' ∗ @{V'} EStackLocal_inner N γg s M' ∗
          if b then (
            (* successful case *)
...
Definition last_state_xchg N γg s γgx γh (xstlist : list xchg_state) (hmap : gmap event_id (gname * view)) : vProp :=
  ∃ xstl xstr, ⌜ last xstlist = Some (xstl, xstr) ⌝ ∗
    ([∗ map] xe↦xinfo ∈ xstl,
      ∃ (γi γpt γpf γu : gname) (Vi : view) M Q,
        ⌜ γi = encode (γpt,γpf,γu) ∧ Vi ⊑ xinfo.1.2 ∧ (0 < xinfo.1.1)%Z ⌝ ∗
        @{xinfo.1.2} (atomic_update_push N γg s xinfo.1.1 Vi M Q ∗ EStackLocal_inner N γg s M) ∗
        OmoTokenW γgx xe ∗
...
Proof.
  iIntros (??????) "M●". by iDestruct (OmoAuth_Linearizable with "M●") as %?.
Qed.
...
Proof.
  iIntros (??????) "M●". iFrame. iIntros "M●". iFrame.
Qed.

Lemma EStackLocal_OmoEview_instance :
  ∀ N γg s M, EStackLocal N γg s M ⊢ OmoEview γg M.
Proof. intros. by iDestruct 1 as "[(%&%&%&%&%&%&%&%&%&%&%& _ & M◯ & _) _]". Qed.

Lemma EStackLocal_Eview_update_instance :
  ∀ N γg s M1 M2, EStackLocal N γg s M1 -∗ OmoEview γg M2 -∗ EStackLocal N γg s (M1 ∪ M2).
Proof.
  intros. iDestruct 1 as "[(%&%&%&%&%&%&%&%&%&%&%& H1 & ⊒M1 & H2) H3]". iIntros "⊒M2".
  iDestruct (OmoEview_union with "⊒M1 ⊒M2") as "⊒M". iFrame "H3". repeat iExists _. iFrame.
Qed.

Lemma new_stack_spec :
  spec_elim_composition.new_stack_spec' (new_estack new_stack' new_exchanger) EStackLocal EStackInv.
Proof.
  ...
  iDestruct "s↦" as "[s0↦ s1↦]". wp_pures. rewrite -[Z.to_nat 0]/(0%nat) shift_0.
  ...
  iCombine "s0↦ s1↦" as "s↦". iDestruct (view_at_intro_incl with "s↦ ⊒V2") as (V3) "(#⊒V3 & %LeV2V3 & s↦@V3)".
  ...
  iMod (@OmoAuth_alloc _ _ _ _ _ eVinit initst _ _ stack_interpretable with "WCOMMIT") as (γg γs)
    "(M● & #⊒M@V2 & #e0↦te0 & #einit✓eVinit & #einit↪emp & WCOMMIT)"; [by eapply stack_step_Init|done|].
  iDestruct (OmoHbToken_finish with "M●") as "[M● M●']".

  iMod (ghost_map_alloc_empty) as (γh) "Hγh". iMod (CWMono_new γg γgt) as (γm) "MONO".
  ...
  iMod (inv_alloc (estackN N) _ (InternalInv_inner N γg s) with "[-Post M● WCOMMIT]") as "#SInv".
  ...
  iDestruct ("Post" $! γg γs s Minit V3 with "[M● WCOMMIT]") as "HΦ".
  { iFrame "⊒V3 WCOMMIT". iSplitL; [iFrame|]. iSplit; [|done]. iFrame "SInv".
    repeat iExists _. iFrame "HM ⊒M@V2 ⊒Mt@V1 ⊒Mx@V2 einit✓eVinit". subst eVinit. simpl. done. }

  iModIntro. by iApply "HΦ".
Qed.

Lemma try_push_spec :
  spec_elim_composition.try_push_spec' (try_push try_push' new_offer check_offer) EStackLocal EStackInv.
Proof.
  intros N DISJ s tid γg M V0 v NZ. iIntros "#⊒V0 #EStackLocal" (Φ) "AU".
  iDestruct "EStackLocal" as "[(%&%&%&%&%&%&%&%&%&%&%& HM & ⊒M & ⊒Mt & ⊒Mx & e0✓eV0 & ⊒eV0) SInv]".
  ...
  iMod ("Hclose" with "[-AU s0↦ s1↦]") as "_"; [by repeat iExists _; iFrame; iFrame "HM AllEvents1 LASTemp1"|].
  ...
  { (* Peeking case *) iIntros "Mt●2". iModIntro. iFrame "AU s0↦ s1↦". repeat iExists _. iFrame. iFrame "HM AllEvents2 LASTemp2". done. }
  ...
  { (* Push on tstack success: commit here *)
    ...
    iMod "AU" as (????) "[>M●2' [_ Commit]]". iDestruct (OmoAuth_agree with "M●2 M●2'") as %(<- & <- & <- & <-). iCombine "M●2 M●2'" as "M●2".
    ...
    iMod (OmoAuth_insert_last with "M●2 ⊒M@V2 WCOMMIT []") as "(M●2 & #⊒M'@V2 & #psId↦ten2 & #psId✓eVpush & _ & WCOMMIT)".
    { have ? : step psId eVpush stf2 nst; [|done]. apply stack_step_Push; try done. set_solver. }
    ...
    iDestruct (big_sepS_subseteq _ _ M with "MAXpsId") as "MAXpsId'"; [set_solver-|].
    ...
    { rewrite omo_append_w_length /=. lia. }
    iDestruct ("Close" with "Mt●2") as "Mt●2".

    iMod ("Commit" $! true V2 _ E2' _ _ M' with "[M●2' WCOMMIT]") as "HΦ".
    { iFrame "⊒V2 WCOMMIT MAXpsId'". iSplitL "M●2'"; [iFrame|]. iSplit.
      - iSplit; [|by iFrame "SInv"]. repeat iExists _.
        iFrame "HM ⊒eV0@V0' ⊒M'@V2 ⊒Mt@V0' ⊒Mx@V0' e0✓eV0". done.
      - iPureIntro. split_and!; try done; [|set_solver-].
        exists (length omo2). rewrite omo_insert_w_append_w; [|lia]. done. }
    ...
    iDestruct (OmoLt_get_append_w with "M●2 ezfs2✓eVzfs2") as "#ezfs2<psId";
      [by apply lookup_lt_Some in Hezfs2; lia|].

    iModIntro. iSplitR "HΦ".
    { repeat iExists _. iFrame. iFrame "HM". iSplitL "BIG2"; last iSplit; last iSplit.
      - iModIntro. iExists nst,tst2. rewrite Htstlist2'. iSplit; [by rewrite !last_app /=|].
      ...
      - iModIntro. rewrite /AllEvents big_sepL_snoc. iDestruct "AllEvents2" as "[ezfl2↪emp BIG]".
        iFrame "ezfl2↪emp BIG". subst eVpush. simpl. iRight. iExists psId,(length Et2).
        iDestruct (OmoEq_get_from_Einfo with "psId✓eVpush") as "#psId=psId".
      ...
      - iPureIntro. split; [done|]. rewrite omo_append_w_write_op. destruct LASTemp2 as (genzfl2 & Hgenzfl2 & H). exists genzfl2. split.
  ...
  { (* Peeking case *) iIntros "Mx●3". iModIntro. iFrame "AU@V0'' s0↦ s1↦ ". repeat iExists _. iFrame. iFrame "HM AllEvents3 LASTemp3". done. }
  ...
  { (* [new_offer] failed, commit nothing *)
    ...
    iMod ("Commit" $! false V0' γs3' E3'' _ _ M with "[M●3']") as "HΦ".
    { iFrame "⊒V0' M●3'". iSplit; [|done]. iSplit; [|by iFrame "SInv"]. repeat iExists _. iFrame "HM ⊒M@V0' ⊒Mt@V0' ⊒Mx@V0' e0✓eV0 ⊒eV0@V0'". }
  ...
  { iDestruct (view_at_objective_iff _ V0'' with "SInv") as "SInv@V0''".
    iCombine "SInv@V0'' AU@V0''" as "RES". iApply (view_at_mono with "RES"); [done|]. iIntros "[#SInv AU]".
    rewrite /atomic_update_push. iAuIntro. iApply (aacc_aupd with "AU"); [done|].
    iIntros (γs E omo stlist) "H". iAaccIntro with "H"; [iIntros "H !>"; iFrame; iIntros "AU !>"; iFrame|].
    iIntros (b V' γs' E' omo' stlist' M') "(⊒V' & H & LocalInner & CASE)". iRight. iModIntro.
    iExists b,V',γs',E',omo',stlist',M'. iFrame. iFrame "SInv". iSplit; [done|]. iIntros "HΦ !>". by iApply "HΦ". }

  (* Obtain latest state and corresponding information of exchanger *)
  iDestruct (ExchangerInv_OmoAuth_acc with "Mx●3") as "[Mx●3 Close]".
  iDestruct (OmoAuth_wf with "Mx●3") as %[OMO_GOODx3' _]. iDestruct ("Close" with "Mx●3") as "Mx●3".
  ...
  { repeat iExists _. iFrame. iFrame "HM AllEvents3 LASTemp3". iModIntro. iSplit.
    - repeat iExists _. iSplit; [by rewrite Hxstlist3' -Hxst3 last_app|].
      ...
      iExists γi,γpt,γpf,γu,V0,M,(λ b, Φ #b). simpl. iFrame "AU@V3 WCOMMIT Hγpt Hγpf KEY". iSplit.
      + iPureIntro. split_and!; try done. solve_lat.
      + repeat iExists _. iFrame "HM ⊒M@V0' ⊒Mt@V0' ⊒Mx@V0' e0✓eV0 ⊒eV0@V0'".
  ...
  { (* Peeking case *) iIntros "[Mx●4 LASTxchg4]". iModIntro.
    iFrame "Hγu". repeat iExists _. iFrame. iFrame "HM AllEvents4 LASTemp4". done. }

  iIntros (v4 V4 xev4 omox4' xstlist4' Mx4) "(LASTxchg4 & #⊒V4 & >Mx●4 & #⊒Mx4@V4 & [%LeV3V4 %SubMxMx4] & CASE)".
  iDestruct (ExchangerInv_OmoAuth_acc with "Mx●4") as "[Mx●4 Close]".
  iDestruct (OmoAuth_wf with "Mx●4") as %[OMO_GOODx4' _]. iDestruct ("Close" with "Mx●4") as "Mx●4".
  ...
  { (* revoked my offer, commit nothing *)
    ...
    { rewrite last_lookup in LASTxst4.
      replace (Init.Nat.pred (length xstlist4)) with (length xstlist4 - 1) in LASTxst4 by lia.
      rewrite Hxstlist4' EQlenxST4 lookup_app_1_ne; [done|]. lia. }
    { by rewrite lookup_app_1_eq. }
    { rewrite Nat.sub_add; [|lia].
      rewrite Homox4' omo_append_w_write_op omo_write_op_length lookup_app_1_eq. done. }
    ...
    iDestruct (ghost_map_elem_agree with "KEY KEY'") as %H.
    inversion H. rewrite -H1 in Hγi'. encode_agree Hγi. subst γi0 V3. clear H.
    iDestruct (view_at_elim with "⊒V3 AU") as "AU".

    iMod "AU" as (γs E omo stlist) "[>M● [_ Commit]]".
    iMod ("Commit" $! false V4 γs E omo stlist M0 with "[M●]") as "HΦ".
    { iFrame "⊒V4 M●". iSplit; [|done]. iApply (view_at_mono with "⊒M0"); [|done]. done. }
  ...
  iDestruct ("BIGxstr4" with "[Hγu]") as "BIGxstr4"; [repeat iExists _; iFrame "KEY Hγpt'"; iSplit; [done|]; iRight; done|].
  iModIntro. iSplitR "HΦ".
  { repeat iExists _. iFrame. iFrame "HM AllEvents4 LASTemp4". iSplit; [by repeat iExists _; iFrame|].
    iPureIntro. split; [|done]. unfold hmap_valid in *. intros. apply VALhmap4 in H. rewrite app_length /=. lia. }

  iDestruct (saved_vProp_agree with "Hγpt Hγpt'") as "EQ". iIntros "_". wp_pures. simpl in *.
  iDestruct (view_at_mono_2 _ V4 with "HΦ") as "HΦ"; [solve_lat|].
  iDestruct (view_at_elim with "⊒V4 HΦ") as "HΦ". iRewrite "EQ". done.
Qed.

Lemma push_spec :
  spec_elim_composition.push_spec' (push try_push' new_offer check_offer) EStackLocal EStackInv.
Proof.
  intros N DISJ s tid γg M V0 v NZ. iIntros "#⊒V0 #EStackLocal" (Φ) "AU".

  iLöb as "IH". wp_rec. awp_apply (try_push_spec with "⊒V0 EStackLocal"); [done|done|].
  iApply (aacc_aupd with "AU"); [done|].
  iIntros (γs E omo stlist) "M●". iAaccIntro with "M●"; first by eauto with iFrame.

  iIntros (b V1 γs' E' omo' stlist' M1) "(⊒V1 & M● & ⊒M1@V1 & CASE)". iModIntro. destruct b.
  - iRight. iDestruct "CASE" as "((%HE' & %Homo' & %LeV0V1 & %SubMM1) & MAX & WCOMMIT)".
    iExists V1,γs',omo',stlist',M1. subst E'. iFrame. iSplit; [done|]. iIntros "HΦ !> _". wp_pures. by iApply "HΦ".
  - iLeft. iDestruct "CASE" as %(-> & -> & -> & -> & ->). iFrame. iIntros "AU !> _". wp_if. iApply ("IH" with "AU").
Qed.

Lemma try_pop_spec :
  spec_elim_composition.try_pop_spec' (try_pop try_pop' take_offer) EStackLocal EStackInv.
Proof.
  intros N DISJ s tid γg M V0. iIntros "#⊒V0 #EStackLocal" (Φ) "AU".
  iDestruct "EStackLocal" as "[(%&%&%&%&%&%&%&%&%&%&%& HM & ⊒M & ⊒Mt & ⊒Mx & e0✓eV0 & ⊒eV0) SInv]".
  wp_lam. wp_pures. rewrite -[Z.to_nat 0]/(0%nat) shift_0. wp_bind (! _)%E.
  ...
  replace (Mt ∪ Mt1) with Mt1; [|set_solver]. iCombine "⊒M ⊒Mt1' ⊒Mx ⊒eV0" as "⊒INFO".
  iDestruct (view_at_intro_incl with "⊒INFO ⊒V0") as (V0') "(⊒V0' & %LeV0V0' & (⊒M@V0' & ⊒Mt1@V0' & ⊒Mx@V0' & ⊒eV0@V0'))".

  iMod ("Hclose" with "[-AU s0↦ s1↦ M⊢M' M'⊢Mt1]") as "_"; [by repeat iExists _; iFrame; iFrame "HM AllEvents1 LASTemp1"|].
  wp_read. iApply fupd_mask_intro_subseteq; [done|]. wp_pures.
  ...
  iDestruct "H" as (γs2 ???????? Et2) "(%Ex2 & %omo2 & %omot2 & %omox2 & %stlist2 & %tstlist2 & %xstlist2 & %Mono2 & %hmap2 & %ezfs2 & %ezfl2 &
    >HM' & >M●2 & >Mt●2 & Mx●2 & >MONO2 & >Hγh2 & >Field2 & >LASTst2 & LASTxchg2 & >#AllEvents2 & >#LASTemp2 & >[%VALhmap2 %LASTemp2])".
  simplify_meta with "HM' HM". iClear "HM'".
  iDestruct (StackInv_OmoAuth_acc with "Mt●2") as "[Mt●2 Close]". iDestruct (OmoAuth_wf with "Mt●2") as %[OMO_GOODt2 _].
  iDestruct (OmoAuth_omo_nonempty with "Mt●2") as %Nomot2. iDestruct ("Close" with "Mt●2") as "Mt●2".
  have NZomot2 : length omot2 ≠ 0 by destruct omot2. eapply omo_stlist_length in OMO_GOODt2 as EQlentST2.

  iAaccIntro with "Mt●2".
  { (* Peeking case *) iIntros "Mt●2". iModIntro. iFrame "AU s0↦ s1↦ M⊢M' M'⊢Mt1". repeat iExists _. iFrame. iFrame "HM AllEvents2 LASTemp2". done. }
  ...
  { (* If there was no race, then commit here, either Pop or EmpPop *)
    iDestruct "CASE" as "([%LeV0'V2 %SubMt1Mt2] & #MAXgenMt1 & CASE)". destruct (decide (v2 = 0%Z)) as [->|NEv2'].
    - (* Commit EmpPop *)
      ...
      { iDestruct "LASTemp2" as (tezf2 tgenzf2) "(#ezfs2↦tezf2 & #MONO✓ezfs2 & (%Htezf2 & %Htgenzf2st & %LASTemp))".
        ...
        { have [st Hst] : is_Some (tstlist2 !! tgen2) by rewrite lookup_lt_is_Some -EQlentST2.
          ...
          - rewrite lookup_omo_ro_event Homot2' omo_insert_r_read_op list_lookup_alter.
            rewrite /omo_read_op list_lookup_fmap Htgen2 /= lookup_app_1_eq. done.
      ...
      iMod (OmoAuth_insert_ro with "M●2 ⊒M@V2 RCOMMIT ezf2↪emp MAXgenM []") as (gen2)
        "(M●2 & #⊒M2@V2 & #ppId↦ten2 & #ezf2=ppId & _ & RCOMMIT & (%eidx & %Heidx & %EQgen))".
      { have ? : step ppId eVpop [] []; [|done]. apply stack_step_EmpPop; try done. set_solver. }
      iDestruct (OmoHbToken_finish with "M●2") as "[M●2 M●2']".
      iDestruct (OmoLe_get_from_Eq with "ezf2=ppId") as "ezf2≤ppId".
      iDestruct (OmoUB_mono with "MAXgenM ezf2≤ppId") as "MAXppId".

      iMod ("Commit" $! 0%Z V2 γs2 E2' _ _ M2 with "[M●2' RCOMMIT]") as "HΦ".
      { iFrame "⊒V2 M●2' RCOMMIT MAXppId". iSplit; last iSplit.
        - iFrame "SInv". repeat iExists _. iFrame "HM ⊒M2@V2 ⊒Mt1@V0' ⊒Mx@V0' e0✓eV0 ⊒eV0@V0'".
        - iPureIntro. simpl. split_and!; try done. set_solver.
        - iPureIntro. split_and!; try done. exists gen2. split; [done|]. apply lookup_omo_lt_Some in Heidx.
          rewrite -EQgen omo_insert_r_length in Heidx. done. }
    ...
    - (* Commit Pop *)
      ...
      iDestruct (OmoEinfo_get _ _ _ _ _ _ (length Et2) with "Mt●2") as "#ten2✓teV2"; [by rewrite HEt2' lookup_app_1_eq|].
      iDestruct ("Close" with "Mt●2") as "Mt●2".
      iDestruct (OmoAuth_wf with "M●2") as %[OMO_GOOD2 _]. eapply omo_stlist_length in OMO_GOOD2 as EQlenST2.
      ...
      { by rewrite HEt2' lookup_app_1_eq. }
      { rewrite Nat.sub_add; [|lia]. rewrite Homot2' omo_append_w_write_op omo_write_op_length lookup_app_1_eq. done. }
      inversion STEP; [inversion PUSH|inversion POP|inversion EMPPOP|inversion INIT].
      subst o oV stk v tstf2. rename tst2 into ntstf2. simpl in *.

      destruct stf2 as [|stf2 nstf2]; [by iDestruct (big_sepL2_length with "BIG2") as %H; simpl in H; lia|].
      iDestruct (big_sepL2_cons with "BIG2") as "[H BIG2]". iDestruct (big_sepL2_length with "BIG2") as %EQlenSTs.
      ...
      iDestruct (OmoAuth_agree with "M●2 M●2'") as %(<- & <- & <- & <-).
      iCombine "M●2 M●2'" as "M●2".
      iMod (OmoAuth_insert_last with "M●2 ⊒M2@V2 WCOMMIT []") as "(M●2 & #⊒M2'@V2 & #ppId↦ten2 & #ppId✓eVpop & #ppId↪nstf2 & WCOMMIT)".
      ...
      iDestruct (OmoHbToken_finish with "M●2") as "[M●2 M●2']".
      iDestruct (OmoUB_singleton with "ppId✓eVpop") as "MAXppId".
      iDestruct (big_sepS_subseteq _ _ M with "MAXppId") as "MAXppId'"; [set_solver-|].
      ...
      iDestruct (OmoEinfo_get _ _ _ _ _ _ ezfs2 with "M●2") as "#ezfs2✓eVzfs2"; [rewrite lookup_app_1_ne; [done|]; apply lookup_lt_Some in Hezfs2; lia|].
      iDestruct (OmoLt_get_append_w with "M●2 ezfs2✓eVzfs2") as "#ezfs2<ppId"; [by apply lookup_lt_Some in Hezfs2; lia|].

      iMod ("Commit" $! v2 V2 γs2 E2' _ _ M2' with "[M●2' WCOMMIT]") as "HΦ".
      { do 2 (rewrite decide_False; [|lia]). iFrame "⊒V2 M●2' WCOMMIT MAXppId'". iSplit; last iSplit.
        - iFrame "SInv". repeat iExists _. iFrame "HM ⊒M2'@V2 ⊒Mt2@V2 ⊒Mx@V0' e0✓eV0 ⊒eV0@V0'".
        - iPureIntro. split_and!; try done. set_solver-.
        - iPureIntro. split_and!; try done. exists (length omo2). rewrite omo_insert_w_append_w; done. }
      ...
      { destruct ntstf2; subst ezfs2' ezfl2'; (iSplit; last iSplit).
        - destruct nstf2; [|done]. iFrame "ppId↪nstf2".
          ...
          + iLeft. iDestruct (OmoEq_get_from_Einfo with "ppId✓eVpop") as "#ppId=ppId". iApply OmoLe_get_from_Eq. done.
        ...
        - iPureIntro. rewrite omo_append_w_write_op.
          destruct LASTemp2 as (genzfl2 & Hezfl2 & LASTemp2). exists genzfl2. split.
  ...
  iDestruct "CASE" as %(-> & -> & -> & ->).
  iModIntro. iSplitR "AU s0↦ s1↦ M⊢M' M'⊢Mt1"; [by repeat iExists _; iFrame; iFrame "HM AllEvents2 LASTemp2"|].
  ...
  iDestruct "H" as (γs3 ???????? Et3) "(%Ex3 & %omo3 & %omot3 & %omox3 & %stlist3 & %tstlist3 & %xstlist3 & %Mono3 & %hmap3 & %ezfs3 & %ezfl3 &
    >HM' & >M●3 & >Mt●3 & >Mx●3 & >MONO3 & >Hγh3 & >Field3 & >#LASTst3 & LASTxchg3 & >#AllEvents3 & >#LASTemp3 & >[%VALhmap3 %LASTemp3])".
  simplify_meta with "HM' HM". iClear "HM'".
  iDestruct (ExchangerInv_OmoAuth_acc with "Mx●3") as "[Mx●3 Close]". iDestruct (OmoAuth_wf with "Mx●3") as %[OMO_GOODx3 _].
  iDestruct (OmoAuth_omo_nonempty with "Mx●3") as %Nomox3. iDestruct ("Close" with "Mx●3") as "Mx●3".
  have NZomox3 : length omox3 ≠ 0 by destruct omox3. eapply omo_stlist_length in OMO_GOODx3 as EQlenxST3.
  ...
  { (* Peeking case *)
    iIntros "[Mx●3 LASTxchg3]". iModIntro. iFrame "AU@V0'' s0↦ s1↦ M⊢M' M'⊢Mt1". repeat iExists _. iFrame. by iFrame "HM AllEvents3 LASTemp3 LASTst3". }
  ...
  { (* [take_offer] failed. [try_pop] also fails. Commit nothing *)
    ...
    iDestruct (view_at_elim with "⊒V0'' AU@V0''") as "AU". iMod "AU" as (γs E omo stlist) "[>M●3' [_ Commit]]".
    iMod ("Commit" $! (-1)%Z V0' γs E omo stlist M with "[M●3']") as "HΦ".
    { iFrame "⊒V0' M●3'". iSplit; [|done]. iFrame "SInv". repeat iExists _. iFrame "HM ⊒M@V0' ⊒Mx@V0' ⊒Mt1@V0' e0✓eV0 ⊒eV0@V0'". }
  ...
  eapply (omo_write_op_step _ _ _ (length omox3 - 1) (length Ex3)) in OMO_GOODx3' as STEP; last first.
  { rewrite Nat.sub_add; [|lia]. rewrite EQlenxST3 Hxstlist3' lookup_app_1_eq. done. }
  { rewrite last_lookup in LASTxst3.
    replace (Init.Nat.pred (length xstlist3)) with (length xstlist3 - 1) in LASTxst3 by lia.
    rewrite Hxstlist3' EQlenxST3 lookup_app_1_ne; [done|]. lia. }
  { by rewrite HEx3' lookup_app_1_eq. }
  { rewrite Nat.sub_add; [|lia]. rewrite Homox3' omo_append_w_write_op omo_write_op_length lookup_app_1_eq. done. }
  inversion STEP; [inversion INIT|inversion REG|inversion ACK|inversion RVK|inversion ACP]. subst e eV stl str v' v. rename H1 into Hxst3.
  ...
  { iDestruct "AU'" as "[AU' ⊒M0]".
    ...
    iAssert (@{V' ⊔ V3} ⊒(V' ⊔ V3))%I with "[]" as "#⊒V@V"; [done|]. iCombine "RES AU@V AU'@V ⊒V@V" as "RES".
    rewrite -view_at_fupd. iApply (view_at_mono with "RES"); [done|].
    iIntros "((#⊒M@V0' & #MONO✓ezfs3 & M●3 & #⊒M0@V' & WCOMMIT2 & WCOMMIT1 & #AllEvents3 &
      #ezfl3↪emp & #HM & #SInv & #LASTst3) & AU2 & AU1 & #⊒V3')".
    ...
    iDestruct "⊒M0@V3'" as (??????????) "(%eV0 & HM' & ⊒M0@V3' & ⊒Mt@V3' & ⊒Mx@V3' & #e0✓eV0 & ⊒eV0@V3')".
    ...
    apply INCL in e3IN as e3IN'. eapply lookup_omo_surjective in e3IN'; [|done].
    destruct e3IN' as [eidx3 Heidx3].
    ...
    - (* If current observation is ealier than [genzfl3], then commit Push and Pop consecutively at [genzfl3] *)
      ...
      { iIntros "%len". iInduction len as [|] "IH".
        - rewrite take_0. iExists [[(psId,v3,V3',M3')]]. iPureIntro. split.
          + rewrite -(app_nil_l [(psId, [])]) -(app_nil_l [[(psId, v3, V3', M3')]]). eapply interp_omo_snoc. split_and!; try done;
              [by rewrite lookup_app_1_eq|by apply interp_omo_nil|by eapply stack_step_Push; try done; set_solver-].
        ...
        - iDestruct ("IH" with "M●3") as %(stnew & IH1 & IH2).
          ...
          specialize (omo_gen_info _ _ _ _ _ _ OMO_GOOD3 Hlast)
            as (eV & eeVs & st' & EID_MATCH & EEVS_VALID & E3e & Lookup_stlist3 & Interp & ES).
          ...
          + iExists (stnew ++ [(e, v, eV.(sync), eV.(eview)) :: stp']). iPureIntro. split.
            * eapply interp_omo_snoc. split_and!; try done; [..|by eapply stack_step_Push].
          ...
          + iExists (stnew ++ [stk ++ [(psId, v3, V3', M3')]]). iPureIntro. split.
            * eapply interp_omo_snoc. split_and!; try done.
              ...
              -- rewrite last_cons last_lookup -EQlen. replace (Init.Nat.pred (S len)) with len by lia. rewrite Hstp'. done.
              -- rewrite EQstp' -H1.
                 replace (((u, v, V, lV) :: stk) ++ [(psId, v3, V3', M3')])
                   with ((u, v, V, lV) :: (stk ++ [(psId, v3, V3', M3')])); [|by simplify_list_eq].
      ...
      (* Commit Push *)
      iMod "AU1" as (????) "[>M●3' [_ Commit]]".
      iDestruct (OmoAuth_agree with "M●3 M●3'") as %(<- & <- & <- & <-). iCombine "M●3 M●3'" as "M●3".
      iMod (OmoAuth_insert with "M●3 ⊒M3@V3' WCOMMIT1 []") as (γs3') "(M●3 & #⊒M3'@V3' & _ & #psId✓eVpush & WCOMMIT)".
      { iPureIntro. split_and!; try done; by subst eVpush. }
      iDestruct (OmoHbToken_finish with "M●3") as "[M●3 M●3']".
      iDestruct (OmoUB_singleton with "psId✓eVpush") as "MAXpsId".
      iDestruct (big_sepS_subseteq _ _ M0 with "MAXpsId") as "MAXpsId'"; [set_solver-|].

      iMod ("Commit" $! true V3' γs3' E3' _ _ M3' with "[M●3' WCOMMIT]") as "HQ".
      { iFrame "⊒V3' M●3' WCOMMIT MAXpsId'". iSplit.
        - repeat iExists _. iFrame "HM ⊒M3'@V3' ⊒Mt@V3' ⊒Mx@V3' e0✓eV0 ⊒eV0@V3'".
      ...
      { iIntros "%len". iInduction len as [|] "IH".
        - rewrite take_0. iExists [[]]. iPureIntro. split.
          + rewrite -(app_nil_l [(ppId, [])]) -(app_nil_l [[]]). eapply interp_omo_snoc. split_and!; try done;
              [by rewrite lookup_app_1_eq|by apply interp_omo_nil|by eapply stack_step_Pop; try done; set_solver-].
          + intros. destruct i; [|done]. inversion H0. subst st2. rewrite Nat.add_0_r Hst in H. inversion H. done.
        - iDestruct ("IH" with "M●3") as %(stnew' & IH1 & IH2).
          ...
          { rewrite lookup_lt_is_Some. apply lookup_lt_Some in Lookup_stnew.
            apply lookup_lt_Some in Hgenzfl3. rewrite -omo_write_op_length in Hgenzfl3.
      ...
      iMod "AU2" as (????) "[>M●3' [_ Commit]]".
      iDestruct (OmoAuth_agree with "M●3 M●3'") as %(<- & <- & <- & <-). iCombine "M●3 M●3'" as "M●3".
      iMod (OmoAuth_insert with "M●3 ⊒M3'@V3' WCOMMIT2 []") as (γs3'') "(M●3 & #⊒M3''@V3' & _ & #ppId✓eVpop & WCOMMIT)".
      { iPureIntro. split_and!; try done; try (by subst eVpop).
        - subst stlist3'.
          ...
          rewrite lookup_app_r; [|lia]. rewrite EQ. replace (gen - gen) with 0 by lia. rewrite -(Nat.add_0_r genzfl3) in Hst.
      ...
      iMod ("Commit" $! v3 V3' γs3'' E3'' _ _ M3'' with "[M●3' WCOMMIT]") as "HΦ".
      { do 2 (rewrite decide_False; [|simpl in *;lia]). iFrame "⊒V3' M●3' WCOMMIT MAXppId'". iSplit.
        - iFrame "SInv". repeat iExists _. iFrame "HM ⊒M3''@V3' ⊒Mt@V3' ⊒Mx@V3' e0✓eV0 ⊒eV0@V3'".
        - iPureIntro. split_and!; try done; [set_solver-|]. by eexists. }
      ...
      + iFrame "ppId↪emp". subst E3'' E3'. rewrite !big_sepL_snoc. iSplit; [iSplit; [|by iLeft]|].
        * iApply big_sepL_forall. iIntros "%e %eV %E3e". iDestruct (big_sepL_lookup with "AllEvents3") as "[CASE|CASE]";
            [done|iLeft; by iApply OmoLe_trans|by iRight].
    ...
    - (* If current observation is later than [genzfl3], then commit Push and Pop consecutively at the end of generation *)
      ...
      iMod "AU1" as (????) "[>M●3' [_ Commit]]".
      iDestruct (OmoAuth_agree with "M●3 M●3'") as %(<- & <- & <- & <-). iCombine "M●3 M●3'" as "M●3".
      iMod (OmoAuth_insert_last with "M●3 ⊒M3@V3' WCOMMIT1 []") as "(M●3 & #⊒M3'@V3' & _ & #psId✓eVpush & _ & WCOMMIT)".
      ...
      { iFrame "⊒V3' M●3' WCOMMIT MAXpsId'". iSplit.
        - repeat iExists _. iFrame "HM ⊒M3'@V3' ⊒Mt@V3' ⊒Mx@V3' e0✓eV0 ⊒eV0@V3'".
        - iPureIntro. split_and!; try done; [|set_solver-]. exists (length omo3). rewrite omo_insert_w_append_w; done. }

      (* Commit Pop *)
      iMod "AU2" as (????) "[>M●3' [_ Commit]]".
      iDestruct (OmoAuth_agree with "M●3 M●3'") as %(<- & <- & <- & <-). iCombine "M●3 M●3'" as "M●3".
      iMod (OmoAuth_insert_last with "M●3 ⊒M3'@V3' WCOMMIT2 []") as "(M●3 & #⊒M3''@V3' & _ & #ppId✓eVpop & _ & WCOMMIT)".
      ...
      { do 2 (rewrite decide_False; [|simpl in *;lia]). iFrame "⊒V3' M●3' WCOMMIT MAXppId'". iSplit.
        - iFrame "SInv". repeat iExists _. iFrame "HM ⊒M3''@V3' ⊒Mt@V3' ⊒Mx@V3' e0✓eV0 ⊒eV0@V3'".
        - iPureIntro. split_and!; try done; [set_solver-|]. exists (length (omo_append_w omo3 (length E3) [])). by rewrite omo_insert_w_append_w. }
      ...
      + iPureIntro. exists genzfl3. split.
        * rewrite !omo_append_w_write_op lookup_app_l; [|rewrite app_length /=; apply lookup_lt_Some in Hgenzfl3; lia].
  ...
  iDestruct (view_at_elim with "⊒V3 HΦ") as "HΦ". iIntros "_". wp_pures. by iApply "HΦ".
Qed.

Lemma pop_spec :
  spec_elim_composition.pop_spec' (pop try_pop' take_offer) EStackLocal EStackInv.
Proof.
  intros N DISJ s tid γg M V0. iIntros "#⊒V0 #EStackLocal" (Φ) "AU".

  iLöb as "IH". wp_rec. awp_apply (try_pop_spec with "⊒V0 EStackLocal"); [done|].
  iApply (aacc_aupd with "AU"); [done|]. iIntros (γs E omo stlist) "M●". iAaccIntro with "M●"; first by eauto with iFrame.

  iIntros (v1 V1 γs' E' omo' stlist' M1) "(⊒V1 & M● & ⊒M1@V1 & CASE)". iModIntro. destruct (decide (v1 = (-1)%Z)) as [->|NEv1].
  - iLeft. iDestruct "CASE" as %(-> & -> & -> & -> & ->). iFrame. iIntros "AU !> _". wp_let. wp_op. wp_if. iApply ("IH" with "AU").
  - iRight. iExists v1,V1,γs',E',omo',stlist',M1. iFrame. iIntros "HΦ !> _". wp_pures.
    rewrite bool_decide_false; [|lia]. wp_pures. by iApply "HΦ".
Qed.

End Elimstack.

Definition elim_stack_impl `{!noprolG Σ, !atomicG Σ, !elimG Σ} (ts : stack_spec Σ) (xc : xchg_spec Σ)
  : spec_elim_composition.stack_spec Σ := {|
    spec_elim_composition.StackInv_Linearizable := EStackInv_Linearizable_instance;
    spec_elim_composition.StackInv_OmoAuth_acc := EStackInv_OmoAuth_acc_instance;
    spec_elim_composition.StackLocal_OmoEview := EStackLocal_OmoEview_instance ts xc;
    spec_elim_composition.StackLocal_Eview_update := EStackLocal_Eview_update_instance ts xc;
    spec_elim_composition.new_stack_spec := new_stack_spec ts xc;
    spec_elim_composition.try_push_spec := try_push_spec ts xc;
    spec_elim_composition.push_spec := push_spec ts xc;
    spec_elim_composition.try_pop_spec := try_pop_spec ts xc;
    spec_elim_composition.pop_spec := pop_spec ts xc;
  |}.
