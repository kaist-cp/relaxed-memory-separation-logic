...
Proof.
  ...
  { (* Read None value from next field. [EmpDeq] case *)
    ...
    { iApply big_sepS_forall. iIntros "%e %eM".
      ...
      - iAssert (∃ gen γl γsl v l, ⌜ CL0 !! gen = Some (e, γl, γsl, v, l) ⌝)%I with "[-]" as (????) "(% & %HCL0gen)".
        { iDestruct "M_CL0" as "[M_CL0 M_CL0']".
          ...
          destruct (CL0 !! gen) eqn:Heq; [|done]. destruct p as [[[[p γl] γsl] v'] l]. inversion Hgen. subst p.
      ...
      - iDestruct "eInfo" as (???) "(e⊒eh & e=e' & eh=eh' & e'↦eh' & MONO✓e')".
        ...
        iApply OmoEq_Le_trans; [done|]. iApply (CWMono_acc with "MONO2 MONO✓e' MONO✓deqid1 e'↦eh' deqid1↦eh1 eh'≤eh1").
      - iDestruct "eInfo" as (???) "(e⊒eh & e=e' & eh=eh' & e'↦eh' & MONO✓e')".
        iDestruct (OmoHb_HbIncluded with "e⊒eh M⊢Mh'") as %ehMh'; [done|].
        iDestruct (big_sepS_elem_of with "MAXgen_eh1") as "#eh≤eh1"; [exact ehMh'|].
        iDestruct (OmoEq_Le_trans with "[] eh≤eh1") as "eh'≤eh1"; [by iApply OmoEq_sym|].
    ...
    iMod (OmoAuth_insert_ro with "M●2 M◯'' RCOMMIT deqid1↪st1 MAXgen_deqid1 []")
      as (gen) "(M●2 & #M◯n & #deqId↦el1n & #deqid1=deqId & #deqId✓egV' & RCOMMIT & _)".
    ...
    iDestruct (OmoLe_get_from_Eq with "deqid1=deqId") as "deqid1≤deqId".
    iDestruct (OmoUB_mono with "MAXgen_deqid1 deqid1≤deqId") as "MAXdeqId".

    iMod ("Commit" $! 0%Z V2 EmpDeq γs2 E2' _ _ M' with "[M●2' RCOMMIT]") as "HΦ".
    { iFrame "⊒V2 M●2' RCOMMIT MAXdeqId". iSplit.
    ...
    { iDestruct ("AllNodesClose" with "[Ml1● Fieldl1 Hgenh1 omol1↦]") as "AllNodes2".
      ...
      rewrite omo_insert_r_write_op. iFrame "∗". iFrame "HM AllPrevLinks2". iSplitL; [eauto with iFrame|].
  ...
  { iDestruct ("AllNodesClose" with "[H3 H4 H5 H6]") as "AllNodes2".
    ...
    iExists E2. iExists γh,γt,γs2,γsh,γst,γcl,γm,Eh2. iExists Et2,omo2,omoh2,omot2,stlist2,moh2,mot2,CL2. iExists _.
  ...
  { (* CAS failed, FAIL_RACE case *)
    ...
    iMod "AU" as (γs E omo stlist) "[>M●3 [_ Commit]]".
    iMod ("Commit" $! FAIL_RACE V3 dummy_qevt γs E _ _ M with "[M●3]") as "HΦ".
    { iFrame "⊒V3 M●3". iSplit; [|done]. repeat iExists _. by iFrame "HM CL◯0 M_CL0 M◯' QInv Mh◯3 Mt◯'". }
  ...
  iDestruct (OmoEinfo_agree with "eh3✓eVh3 eh3'✓eVh3'") as %<-. iClear "deqid3'↦eh3' deqid3'↪st3' eh3'✓eVh3'".
  ...
  { iIntros "%g1 %g2 %qu1 %qu2 %Lookup1 %Lookup2 %LE1 %LE2".
    ...
    - replace (S m) with (m + 1) in Lookup2; [|lia].
      ...
      { eapply (lookup_omo_event_valid _ _ _ (wr_event (m + 1))); [done|]. by rewrite lookup_omo_wr_event. }
  ...
  { iIntros "%g %st %Hst %LE".
    ...
    - clear st' Hstgen.
      ...
      { rewrite lookup_lt_is_Some. apply lookup_lt_Some in Hstdropdrop. by rewrite !fmap_length in Hstdropdrop. }
      destruct ninfo as [[[[e' γl] γsl] v] l].
  ...
  have bLeV0V3 : bool_decide (V0 ⊑ V3) by eapply bool_decide_unpack; eauto.
  set deqId := length E3.
  ...
  iMod (OmoAuth_insert with "M●3 M◯3'' WCOMMIT []") as (γs3') "(M●3 & #M◯3''' & #deqId↦ehn3 & #deqId✓egV' & WCOMMIT)".
  ...
  { subst omoh3'. rewrite omo_append_w_write_op /AllHeadMsg big_sepL_snoc. iSplit.
    ...
    - iAssert (∃ Ml2, OmoAuth γg γs3' (1/2)%Qp (E3 ++ [egV']) omo3' (take (gen + 1) stlist3 ++ stlist3') _ ∗
  ...
  iDestruct (big_sepS_subseteq _ _ M with "MAXgen_deqId") as "MAXdeqId"; [set_solver-|].
  iMod ("Commit" $! zl2 V3 (Deq zl2) γs3' (E3 ++ [egV']) _ _ Mdeq with "[M●3' WCOMMIT]") as "HΦ".
  { do 2 (rewrite decide_False; [|lia]). iFrame "⊒V3 M●3' WCOMMIT MAXdeqId". iSplit.
    - repeat iExists _.
    ...
    - iPureIntro. split_and!; try done; [set_solver-|lia|]. by eexists. }
...
Proof.
  intros N DISJ q tid γg M0 V0. iIntros "#⊒V0 #M◯0" (Φ) "AU".
  iLöb as "IH". wp_lam.
  awp_apply (try_deq_spec with "⊒V0 M◯0"); [eauto..|].
  iApply (aacc_aupd with "AU"); [done|].
  iIntros (γs E omo stlist) "QInv".
  iAaccIntro with "QInv"; [eauto with iFrame|].
  iIntros (z V1 deq γs' E' omo' stlist' M1) "(⊒V1 & QInv & ⊒M1@V1 & CASE) !>".

  destruct (decide (z = (-1)%Z)) as [->|NEQ].
  - iLeft. iDestruct "CASE" as %(-> & -> & -> & -> & ->). iFrame. iIntros "AU !> _". wp_pures.
    rewrite bool_decide_false; [|lia]. by iApply ("IH" with "AU").
  - iRight. iDestruct "CASE" as "((%HE' & %LeV0V1 & %SubM0M1) & MAX & CASE)".
    iExists z,V1,deq,γs',omo',stlist',M1. subst E'.
    iAssert (⌜ (0 ≤ z)%Z ⌝)%I with "[-]" as %LE.
    { destruct (decide (z = 0%Z)) as [->|NEz0]; [iPureIntro; lia|].
      iDestruct "CASE" as "[[%GZ _] _]". iPureIntro. lia. }
    iFrame. iSplit; [done|]. iIntros "HΦ !> _". wp_pures. rewrite bool_decide_true; [|lia]. wp_pures. by iApply "HΦ".
Qed.
End msqueue.

Definition msqueue_impl_history `{!noprolG Σ, !atomicG Σ, !msqueueG Σ}
  : queue_spec Σ := {|
    spec_composition.QueueInv_Linearizable := QueueInv_Linearizable_instance;
    spec_composition.QueueInv_OmoAuth_acc := QueueInv_OmoAuth_acc_instance;
    spec_composition.QueueLocal_OmoEview := QueueLocal_OmoEview_instance;
    spec_composition.QueueLocal_Persistent := QueueLocal_persistent;
    spec_composition.new_queue_spec := new_queue_spec;
    spec_composition.try_enq_spec := try_enq_spec;
    spec_composition.enqueue_spec := enqueue_spec;
    spec_composition.try_deq_spec := try_deq_spec;
    spec_composition.dequeue_spec := dequeue_spec;
  |}.
